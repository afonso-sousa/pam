...
class PAMGAT(BertPreTrainedModel):
    def __init__(self, config):
    ...
    def forward(
        ...
        # Reshape `dense_adj` to [batch_size, 1, seq_length, seq_length] to match broadcasting needs
...
class GraphSentenceEncoder(BertEncoder):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                (
                    GraphBertLayer(config, add_gnn=True)
                    if layid == config.num_hidden_layers - 1
                    else GraphBertLayer(config)
                )
...
class GraphBertLayer(BertLayer):
    def __init__(self, config, add_gnn=False):
        ...
        if self.add_gnn:
            config.gnn_layer = getattr(config, "gnn_layer", 4)

            self.adapter_graph = nn.ModuleList(
                [GNNAdapter(config) for _ in range(config.gnn_layer)]
            )
            self.graph_cross_attention = BertAttention(config)
            self.graph_projection = nn.Linear(
                config.hidden_size * config.gnn_layer, config.hidden_size
            )
...
class GNNAdapter(nn.Module):
    def __init__(self, config):
        ...
        self.wo = nn.Linear(config.gnn_size * num_heads, config.hidden_size, bias=False)
