...
class GNNAdapter(nn.Module):
    ...
    def forward(self, hidden_states, graphs):
        ...
        # gate = torch.sigmoid(self.gate_layer(hidden_states))  # Shape: [batch, seq, 1]
        # gate = gate.expand_as(hidden_states)
        # layer_output = gate * hidden_states + (1 - gate) * self.dropout(y)
