...
class RobertaMeanPooler(nn.Module):
...
class PAMRoberta(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        self.pooler = RobertaMeanPooler(config)
        self.init_weights()
...
class GraphSentenceEncoder(RobertaEncoder):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.layer = nn.ModuleList(
            [
                GraphRobertaLayer(
                    config,
                    add_gnn=(
                        layid == 0 or layid == config.num_hidden_layers - 1
...
class GraphRobertaLayer(RobertaLayer):
    def __init__(self, config, add_gnn=False):
        ...
        if self.add_gnn:
            ...
            self.graph_cross_attention = RobertaAttention(config)
