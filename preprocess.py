    ...
    if map:
        ...
        try:
            ...
            for idx, tok in enumerate(graph_string_tok):
                # Handle subword prefixes for RoBERTa and BERT
                if is_roberta:
                    clean_tok = tok.lstrip("Ġ")  # Remove RoBERTa's subword marker
                else:
                    clean_tok = tok.replace("##", "")  # Remove BERT's subword marker

                tokenized_node += unidecode.unidecode(clean_tok.lower())
                plain_tokenized_node += clean_tok.lower()

                unaccented_string = unidecode.unidecode(
                    graph_string[idx_graph_string].lower()
                )

                if not unaccented_string.startswith(
                    tokenized_node
                ) and not unaccented_string.startswith(plain_tokenized_node):
                    idx_graph_string += 1
                    map[idx_graph_string] = []
                    tokenized_node = ""
                    plain_tokenized_node = ""
            ...
            raise Exception(f"Error when converting graph to tokenized version: {e}")

    assert len(map) == len(graph_string), f"Graph mapping failed: {graph_string}"
...
def generate_ref_edge(line, tokenizer, max_seq_length):
    is_roberta = (
        "roberta" in tokenizer.__class__.__name__.lower()
        or "roberta" in tokenizer.name_or_path.lower()
    )
    ...
    ref1_graph_string_tok = tokenizer.tokenize(
        f"{cls_token} {graph_ref1['amr_simple']} {sep_token}"
    )
    ref2_graph_string_tok = tokenizer.tokenize(
        f"{cls_token} {graph_ref2['amr_simple']} {sep_token}"
    )
    ref1_triples_graph = align_triples(
        ref1_graph_string_tok,
        f"{cls_token} {graph_ref1['amr_simple']} {sep_token}",
        (
            json.loads(graph_ref1["triples"])
            if type(graph_ref1["triples"]) == str
            else graph_ref1["triples"]
        ),
        1,
        is_roberta=is_roberta,
    )
    ref2_triples_graph = align_triples(
        ref2_graph_string_tok,
        f"{cls_token} {graph_ref2['amr_simple']} {sep_token}",
        (
            json.loads(graph_ref2["triples"])
            if type(graph_ref2["triples"]) == str
            else graph_ref2["triples"]
        ),
        1,
        is_roberta=is_roberta,
    )

    ref1_edge_index, ref1_edge_types, ref1_position_ids = generate_edge_tensors(
        ref1_triples_graph, max_seq_length
    )
    ref2_edge_index, ref2_edge_types, ref2_position_ids = generate_edge_tensors(
        ref2_triples_graph, max_seq_length
    )
...
def generate_camr_edge(line, tokenizer, max_seq_length):
    ...
    cls_token = tokenizer.cls_token or "[CLS]"
    sep_token = tokenizer.sep_token or "[SEP]"
    graph_string_tok = tokenizer.tokenize(
        f"{cls_token} {line['amr_simple']} {sep_token}"
    )
    try:
        triples_graph = align_triples(
            graph_string_tok,
            f"{cls_token} {line['amr_simple']} {sep_token}",
            json.loads(line["triples"]),
            1,
            is_roberta=is_roberta,
        )
        edge_index, edge_types, position_ids = generate_edge_tensors(
            triples_graph, max_seq_length
        )
...
def generate_wiki_aug_edge(graph_triples, positive_graph_triples, max_seq_length):
    ...
    pos_edge_index, pos_edge_types, pos_position_ids = generate_edge_tensors(
        positive_graph_triples, max_seq_length
    )
    return (
        [edge_index, pos_edge_index],
        [edge_types, pos_edge_types],
...
def align_wiki_triples(line, tokenizer):
    ...
    graph_string_tok = tokenizer.tokenize(f"{cls_token} {linearized_graph} {sep_token}")
    tok_map = {}
    count = 0
    graph_string = f"{cls_token} {linearized_graph} {sep_token}"
...
def aligh_wiki_amr(wikipedia_dataset_path, tokenizer):
    ...
    with open(wikipedia_dataset_path, "r", encoding="utf8") as fIn:
        for line in tqdm(fIn):
            ...
            positive_graph_triples = align_wiki_aug_triples(
                positive_linearized_graph, positive_triples, tokenizer
            )
